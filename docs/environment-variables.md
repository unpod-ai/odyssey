...
| `ODYSSEY_INSTRUMENT` | `auto` | What `odyssey.init()` attaches to this process. `auto` (the default) patches every provider SDK actually installed — `anthropic`, `openai` (and any OpenAI-compatible gateway), `google-genai` — and registers LangChain's handler process-wide; `all` adds the OpenTelemetry bridge (excluded from `auto` because a patched client plus an OTel processor records every call twice, and `opentelemetry-sdk` is a common transitive dependency); `none` (or `off`, or an empty value) attaches nothing. Also accepts an explicit comma/space-separated list — `openai,otel` — where a *named* target that is not installed is reported through `odyssey.health()` instead of being skipped silently the way an expanded `auto`/`all` skips it. `livekit` and `pipecat` are not valid here: they attach to an object the application owns (`attach(session, ...)` / `attach(task, ...)`), and naming one says so. Overridden by `odyssey.init(instrument=...)` |
...
| `ODYSSEY_ENDPOINT` | unset | `HttpSink`'s target URL when constructed with no explicit `endpoint` — e.g. `http://127.0.0.1:8787` for a local `services/collector`. Also selects `odyssey.init()`'s **default sink**: set, and drained journeys are POSTed to that collector; unset, they land in `ODYSSEY_OUT` through `FileSink` as before. An unusable endpoint falls back to the file sink and counts the failure in `odyssey.health()` rather than failing `init()` |
