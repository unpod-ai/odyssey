...
(item 8.7) runs `odyssey api openapi` then `odyssey sdk codegen` in
sequence — regenerating both, in dependency order; a new
`codegen-drift.yml` CI workflow runs the `--check`/`check-drift` variants
of the same two steps and fails the build on either kind of drift. New
`ci-sdk.yml` (named exactly as `docs/STRUCTURE.md` specifies, "sdk/**
(py + js matrix)" — only the Python leg runs today; a js job is meant to
be added the same commit `sdk/javascript` lands, not a second workflow
file).
...
`apps/web` (item 8.6) — new Next.js 16 (App Router, TypeScript, npm)
...
actually exposes; "experiments"/"reports" have no backing endpoint.
`src/lib/api/{types,client}.ts` is a hand-written, stdlib-`fetch`-based
stand-in for `@odyssey/sdk` (`sdk/javascript`, item 8.5 — not built this
pass, so `apps/web`'s own `docs/STRUCTURE.md`-specified rule "consumes
`@odyssey/sdk`, NOT its own generated client" is a documented, temporary
scope cut, not the intended end state — see `apps/web/README.md`). Every
page is a React Server Component fetching server-side
(`cache: "no-store"`), no client-side data-fetching library needed for a
read-only dashboard; a failed fetch renders inline rather than crashing.
...
New `ci-web.yml` (lint + vitest + `next build`, which also type-checks
and prerenders every route). **No browser/e2e runner wired up**
(`tests/e2e/` stays empty, documented) — this repo's sandbox has no
browser tool, so verification was instead: a real `services/api`
instance started via `uvicorn`, `npm run dev` pointed at it via
`ODYSSEY_API_BASE_URL`, and `curl` against every route (`/`, `/journeys`,
`/journeys/{id}` including a 404 for a missing journey, `/datasets`) with
the *resolved* server-rendered HTML inspected (React inserts `<!-- -->`
hydration-boundary comments between text nodes, which a naive `grep`
missed at first — worth knowing for anyone else grepping Next.js SSR
output) to confirm real API data actually reached the page, not a
`TestClient`-only check. 3 new vitest unit tests for `src/lib/api/
client.ts` (URL building, path-param encoding, error-status mapping)
against a mocked `fetch` — the one thing that doesn't need a browser to
verify meaningfully.
