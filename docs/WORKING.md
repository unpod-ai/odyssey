...
| 1.6 | **Auth** (API key) + product scoping | ✅ | `HttpSink(api_key=...)` / `ODYSSEY_API_KEY` sends a `Bearer` token. `services/collector` now supports two mutually exclusive auth modes: the original single shared `api_key` (unscoped, unchanged), or a `products` roster (`--products-file`/`ODYSSEY_COLLECTOR_PRODUCTS_FILE`, a JSON `{"products": [{"slug", "name", "api_key"}, ...]}` file) — each registered product has a unique `slug` (what actually names its storage partition, `<data_dir>/<slug>/<date>/...`) and a human-readable `name` (for `GET /products` and operator legibility), so isolation is structural, not just an access check on shared storage. `GET /products` (any registered key) lists `{slug, name}` for the roster, never keys. A stopgap, documented as such: the roster is a flat file loaded once at startup, not a database — real key/product management belongs to `services/api` (Step 8, not built) |
