...
| 1.7 | **Batching / compression / backpressure on the wire** | ✅ | `HttpSink` gzips the JSONL body by default (`Content-Encoding: gzip`, `compress=False` to opt out); the collector decompresses. A 429 response's `Retry-After` (seconds or HTTP-date) sets a client-side backoff window — `send()` raises without a network call if called again before it elapses. **Cross-journey overhead**: still one POST per journey, not one merged POST for N — merging payloads was ruled out (it would need a real redesign of `drain()`'s per-journey watermark/retry semantics to stay correct across a partial failure) in favor of reusing the connection itself: `HttpSink` now holds an `http.client.HTTPConnection` across `send()` calls (HTTP/1.1 keep-alive; `services/collector`'s `_Handler.protocol_version = "HTTP/1.1"` opts in), so draining N journeys in one process pays for one TCP/TLS handshake, not N, with zero change to the per-request watermark-advance correctness — every journey's POST/response stays independent. A dropped keep-alive connection (server idle-timeout, restart) is retried once transparently, not surfaced as a spurious drain failure. `send()` is not safe to call concurrently from two threads on the same sink (shared connection state) — guarded by an internal lock |
...
- **The OTel bridge only understands the official `gen_ai.*` semantic
  ...
  silent data loss.
- **Cross-journey payload batching is not attempted** (1.7). Still one POST
  per journey, not one merged POST for N — a redesign of `drain()`'s
  per-journey watermark/retry semantics would be needed for partial-batch
  failure. `HttpSink` reusing its connection across journeys (HTTP/1.1
  keep-alive) addresses the actual per-journey overhead this item was
  chasing without that redesign — see item 1.7's own row.
